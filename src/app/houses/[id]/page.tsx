...
export default async function HouseDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (!house) {
        return (
            <div className="container">
                ...
                <main className="main" style={{ textAlign: 'center', marginTop: '50px' }}>
                    <h1 className="text-2xl font-bold">Casa no encontrada</h1>
                    <p style={{ marginBottom: '20px' }}>Lo sentimos, el alojamiento que buscas no existe.</p>
                    <Link href={routeConstants.root} className="btnPrimary" style={{ display: 'inline-block', width: 'auto', padding: '10px 20px' }}>
                        Volver al inicio
                    </Link>
                </main>
            </div>
    ...
    return (
        <div className="container">
            <nav className="nav">
                <div className="navContent">
                    <Link href={routeConstants.root} className="logo" style={{ textDecoration: 'none' }}>Renting Houses</Link>
                    <CartCounter />
                </div>
            </nav>
